...
function xmldb_aiknowledgecheck_upgrade($oldversion) {
    ...
    if ($oldversion < 2026030600192) {
        // v1.3.92: Live attempts badge fix — no DB schema changes.
        upgrade_mod_savepoint(true, 2026030600192, 'aiknowledgecheck');
    }

    if ($oldversion < 2026031500197) {
        ...
        upgrade_mod_savepoint(true, 2026031500197, 'aiknowledgecheck');
    }

    if ($oldversion < 2026031600398) {
        ...
        upgrade_mod_savepoint(true, 2026031600398, 'aiknowledgecheck');
    }

    if ($oldversion < 2026031700399) {
        ...
        // No DB schema changes.
        upgrade_mod_savepoint(true, 2026031700399, 'aiknowledgecheck');
    }

    if ($oldversion < 2026031901004) {
        ...
        upgrade_mod_savepoint(true, 2026031901004, 'aiknowledgecheck');
    }

    if ($oldversion < 2026031901005) {
        ...
        upgrade_mod_savepoint(true, 2026031901005, 'aiknowledgecheck');
    }

    if ($oldversion < 2026032200514) {
        // v1.5.14: ETA BANNERS — Estimated Time to Complete banners for teacher + student views.
        // No DB schema changes.
        upgrade_mod_savepoint(true, 2026032200514, 'aiknowledgecheck');
    }

    if ($oldversion < 2026032200515) {
        // v1.5.15: RETRY WRONG ANSWERS — Retake shows only incorrectly answered questions.
        // No DB schema changes.
        upgrade_mod_savepoint(true, 2026032200515, 'aiknowledgecheck');
    }

    // v1.5.16: VERSION BUMP — Maintenance release. No DB changes.
    if ($oldversion < 2026032200516) {
        upgrade_mod_savepoint(true, 2026032200516, 'aiknowledgecheck');
    }

    if ($oldversion < 2026032300518) {
        ...
        upgrade_mod_savepoint(true, 2026032300518, 'aiknowledgecheck');
    }

    if ($oldversion < 2026032300519) {
        ...
        upgrade_mod_savepoint(true, 2026032300519, 'aiknowledgecheck');
    }

    // v1.5.20: Course info time estimation update — 2 min per question.
    if ($oldversion < 2026032300520) {
        upgrade_mod_savepoint(true, 2026032300520, 'aiknowledgecheck');
    }

    // v1.5.21: Attempt-grouped PDF/Text results — JS-only change; no DB schema changes.
    if ($oldversion < 2026032300521) {
        upgrade_mod_savepoint(true, 2026032300521, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026032300522) {
        upgrade_mod_savepoint(true, 2026032300522, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026032300523) {
        upgrade_mod_savepoint(true, 2026032300523, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026032300524) {
        upgrade_mod_savepoint(true, 2026032300524, 'aiknowledgecheck');
    }
    ...
    //          "cannot downgrade" exception (2026032400525 → 2026032300524). Correct order
    //          restored in v1.5.30.
    if ($oldversion < 2026032400525) {
        $table  = new xmldb_table('aiknowledgecheck');
        $field  = new xmldb_field('aftercompletion', XMLDB_TYPE_CHAR, '20', null, XMLDB_NOTNULL, null, 'restart', 'audiominseconds');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_mod_savepoint(true, 2026032400525, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026032400529) {
        upgrade_mod_savepoint(true, 2026032400529, 'aiknowledgecheck');
    }

    // v1.5.30: CRITICAL FIXES —
    //          (1) Savepoint order corrected: v1.5.24 (2026032300524) was listed after
    //              v1.5.25 (2026032400525) in upgrade.php causing a "cannot downgrade"
    ...
    //          No DB schema changes.
    if ($oldversion < 2026032401530) {
        upgrade_mod_savepoint(true, 2026032401530, 'aiknowledgecheck');
    }

    // v1.5.31: VERSION BUMP — Clean release. No DB schema changes.
    if ($oldversion < 2026032401531) {
        upgrade_mod_savepoint(true, 2026032401531, 'aiknowledgecheck');
    }

    // v1.5.32: Industry & Sector dropdown unification. No DB schema changes.
    if ($oldversion < 2026032401532) {
        upgrade_mod_savepoint(true, 2026032401532, 'aiknowledgecheck');
    }
    ...
    //          No DB schema changes.
    if ($oldversion < 2026032601533) {
        upgrade_mod_savepoint(true, 2026032601533, 'aiknowledgecheck');
    }
    ...
    //          No DB schema changes.
    if ($oldversion < 2026032601534) {
        upgrade_mod_savepoint(true, 2026032601534, 'aiknowledgecheck');
    }

    // v1.5.35: BUG FIX — ETA timing and resume-from-all-complete score reconstruction. No DB schema changes.
    if ($oldversion < 2026032601535) {
        upgrade_mod_savepoint(true, 2026032601535, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026032601536) {
        upgrade_mod_savepoint(true, 2026032601536, 'aiknowledgecheck');
    }

    // v1.5.37: VERSION BUMP — Clean release increment. No code or DB schema changes.
    if ($oldversion < 2026032601537) {
        upgrade_mod_savepoint(true, 2026032601537, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026032601538) {
        upgrade_mod_savepoint(true, 2026032601538, 'aiknowledgecheck');
    }

    // v1.5.39: VERSION BUMP — Clean release increment. No code or DB schema changes.
    if ($oldversion < 2026032601539) {
        upgrade_mod_savepoint(true, 2026032601539, 'aiknowledgecheck');
    }

    // v1.5.40: NEW FEATURE — "Add More Questions" button on the teacher ready screen.
    //          No DB schema changes.
    if ($oldversion < 2026032601540) {
        upgrade_mod_savepoint(true, 2026032601540, 'aiknowledgecheck');
    }

    // v1.5.41: BUG FIX — Regenerate Questions double-request fix + AI parse hardening.
    //          No DB schema changes.
    if ($oldversion < 2026032601541) {
        upgrade_mod_savepoint(true, 2026032601541, 'aiknowledgecheck');
    }

    // v1.5.42: VERSION BUMP — Clean release increment. No code or DB schema changes.
    if ($oldversion < 2026032601542) {
        upgrade_mod_savepoint(true, 2026032601542, 'aiknowledgecheck');
    }

    // v1.5.43: BUG FIX — Audio gate fix + Gemini responseMimeType in generationConfig. No DB schema changes.
    if ($oldversion < 2026032701543) {
        upgrade_mod_savepoint(true, 2026032701543, 'aiknowledgecheck');
    }

    // v1.5.44: VERSION BUMP — Clean release increment. No code or DB schema changes.
    if ($oldversion < 2026032701544) {
        upgrade_mod_savepoint(true, 2026032701544, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026032701545) {
        upgrade_mod_savepoint(true, 2026032701545, 'aiknowledgecheck');
    }
    ...
    //   No DB schema changes.
    if ($oldversion < 2026032800146) {
        upgrade_mod_savepoint(true, 2026032800146, 'aiknowledgecheck');
    }

    // v1.5.47: VERSION BUMP — Maintenance release. No code or DB schema changes.
    if ($oldversion < 2026032800147) {
        upgrade_mod_savepoint(true, 2026032800147, 'aiknowledgecheck');
    }

    // v1.5.48: VERSION BUMP — Maintenance release. No code or DB schema changes.
    if ($oldversion < 2026032800148) {
        upgrade_mod_savepoint(true, 2026032800148, 'aiknowledgecheck');
    }

    // v1.5.49: UPGRADE FIX — Corrected upgrade.php savepoint ordering. v1.5.4 block
    //   (2026031901004) was mistakenly inserted AFTER v1.5.5 block (2026031901005) in
    //   db/upgrade.php. Sites upgrading from v1.3.99 or earlier would hit the
    //   2026031901005 savepoint first (setting version to 2026031901005), then attempt
    //   to set 2026031901004 — triggering a fatal "Cannot downgrade" error. Fixed by
    //   moving the v1.5.4 block to its correct position (after v1.3.99, before v1.5.5).
    //   No code, JS, CSS, or DB schema changes. version.php → 2026040100201.
    if ($oldversion < 2026040100201) {
        upgrade_mod_savepoint(true, 2026040100201, 'aiknowledgecheck');
    }
    ...
    //   No DB schema changes. PHP-only fix. version.php → 2026040200150.
    if ($oldversion < 2026040200150) {
        upgrade_mod_savepoint(true, 2026040200150, 'aiknowledgecheck');
    }
    ...
    //   src=build=min triple-match MD5: 4008bb2dc5c3fc7314b5412684fe2979. version.php → 2026040300151.
    if ($oldversion < 2026040300151) {
        upgrade_mod_savepoint(true, 2026040300151, 'aiknowledgecheck');
    }
    ...
    //   src=build=min triple-match MD5: 747cd7c02bbbc25ad2f13d565722502d. version.php → 2026040400152.
    if ($oldversion < 2026040400152) {
        upgrade_mod_savepoint(true, 2026040400152, 'aiknowledgecheck');
    }
    ...
    //   PHP: view.php updated. No DB schema changes. version.php → 2026040800153.
    if ($oldversion < 2026040800153) {
        upgrade_mod_savepoint(true, 2026040800153, 'aiknowledgecheck');
    }
    ...
    //   No DB schema changes. version.php → 2026040800154.
    if ($oldversion < 2026040800154) {
        upgrade_mod_savepoint(true, 2026040800154, 'aiknowledgecheck');
    }
    ...
    // ⚠ FORMAT BUG: 202604090055 is 12-digit. It is numerically LESS than v1.5.54's
    //   savepoint 2026040800154 (13-digit), so any site on v1.5.54 or earlier sees
    //   ($oldversion < 202604090055) as FALSE and silently skips this block. No DB
    //   changes here, so the skip is harmless. Fixed in v1.5.58 by using 13-digit format.
    if ($oldversion < 202604090055) {
        upgrade_mod_savepoint(true, 202604090055, 'aiknowledgecheck');
    }
    ...
    //   DB: adds aiknowledgecheck.showvideoduringquiz. version.php → 202604100056.
    // ⚠ FORMAT BUG: 202604100056 is 12-digit. Same issue as v1.5.55 above — SKIPPED
    ...
    //   DB: adds aiknowledgecheck.showchapterstamps. version.php → 202604100057.
    // ⚠ FORMAT BUG: 202604100057 is 12-digit. Same issue as v1.5.55 above — SKIPPED
    //   for sites on v1.5.54 or earlier. THIS BLOCK CONTAINS A DB SCHEMA CHANGE
    //   (showchapterstamps). Backfilled in v1.5.58 below with field_exists() guards.
    if ($oldversion < 202604100057) {
        $table = new xmldb_table('aiknowledgecheck');
        $field = new xmldb_field('showchapterstamps', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0', 'showvideoduringquiz');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_mod_savepoint(true, 202604100057, 'aiknowledgecheck');
    }

    // v1.5.58: FIX-KC-NUMERIC-VERSION — Corrects the 12-digit savepoint format used in
    //   v1.5.55/56/57. Sites upgrading from v1.5.54 (last 13-digit: 2026040800154) could
    //   not install v1.5.55-57 because those savepoints (202604090055, 202604100056,
    //   202604100057) are numerically LESS than 2026040800154 — Moodle saw them as
    //   "higher version already installed". This block uses 13-digit savepoint 2026040900058
    //   which IS greater than 2026040800154, unblocking upgrades for all affected sites.
    ...
    //   No AMD changes. version.php → 2026040900058.
    if ($oldversion < 2026040900058) {
        $table = new xmldb_table('aiknowledgecheck');
        ...
        upgrade_mod_savepoint(true, 2026040900058, 'aiknowledgecheck');
    }
    ...
    //   No DB schema changes. PHP/JS unchanged. version.php → 2026040900059.
    if ($oldversion < 2026040900059) {
        upgrade_mod_savepoint(true, 2026040900059, 'aiknowledgecheck');
    }
    ...
    //   No DB schema changes. AMD: knowledgecheck.js updated (src=build=min). version.php → 2026041000060.
    if ($oldversion < 2026041000060) {
        upgrade_mod_savepoint(true, 2026041000060, 'aiknowledgecheck');
    }
    ...
    //   No code changes. No DB schema changes. AMD unchanged. version.php → 2026041000061.
    if ($oldversion < 2026041000061) {
        upgrade_mod_savepoint(true, 2026041000061, 'aiknowledgecheck');
    }
    ...
    //   No DB schema changes. AMD unchanged. version.php → 2026041100062.
    if ($oldversion < 2026041100062) {
        upgrade_mod_savepoint(true, 2026041100062, 'aiknowledgecheck');
    }
    ...
    // PHP-only fix in view.php. version.php → 2026041600063.
    if ($oldversion < 2026041600063) {
        upgrade_mod_savepoint(true, 2026041600063, 'aiknowledgecheck');
    }
    ...
    // regeneration. Server-only fix in routes.ts. version.php → 2026041600064.
    if ($oldversion < 2026041600064) {
        upgrade_mod_savepoint(true, 2026041600064, 'aiknowledgecheck');
    }
    ...
    // HTML output. version.php → 2026041700065.
    if ($oldversion < 2026041700065) {
        upgrade_mod_savepoint(true, 2026041700065, 'aiknowledgecheck');
    }
    ...
    // version.php → 2026041700066.
    if ($oldversion < 2026041700066) {
        upgrade_mod_savepoint(true, 2026041700066, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026041700067) {
        upgrade_mod_savepoint(true, 2026041700067, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026042000068) {
        upgrade_mod_savepoint(true, 2026042000068, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026042100069) {
        upgrade_mod_savepoint(true, 2026042100069, 'aiknowledgecheck');
    }
    ...
    //   DB: adds aiknowledgecheck_questions.timestamp_seconds. version.php → 2026042100070.
    if ($oldversion < 2026042100070) {
        $table = new xmldb_table('aiknowledgecheck_questions');
        $field = new xmldb_field('timestamp_seconds', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'mappingcriteria');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_mod_savepoint(true, 2026042100070, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026042100071) {
        // No DB schema changes in this release — bump version only.
        upgrade_mod_savepoint(true, 2026042100071, 'aiknowledgecheck');
    }
    ...
    //   version.php → 2026042100072.
    if ($oldversion < 2026042100072) {
        // No DB schema changes in this release — bump version only.
        upgrade_mod_savepoint(true, 2026042100072, 'aiknowledgecheck');
    }
    ...
    //
    //   No DB schema changes. version.php → 2026042100073.
    if ($oldversion < 2026042100073) {
        // No DB schema changes in this release — bump version only.
        upgrade_mod_savepoint(true, 2026042100073, 'aiknowledgecheck');
    }
    ...
    //   version.php → 2026042100074.
    if ($oldversion < 2026042100074) {
        // No DB schema changes in this release — bump version only.
        upgrade_mod_savepoint(true, 2026042100074, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026042200075) {
        upgrade_mod_savepoint(true, 2026042200075, 'aiknowledgecheck');
    }
    ...
    //   version.php → 2026042300076.
    if ($oldversion < 2026042300076) {
        upgrade_mod_savepoint(true, 2026042300076, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026042300077) {
        upgrade_mod_savepoint(true, 2026042300077, 'aiknowledgecheck');
    }
    ...
    //   version.php → 2026042400078.
    if ($oldversion < 2026042400078) {
        upgrade_mod_savepoint(true, 2026042400078, 'aiknowledgecheck');
    }

    // v1.5.79 - REBUMP: Version bump only. AMD triple-match re-applied (src=build=min,
    //   MD5 9fe7b73f). No JS, PHP, or DB schema changes.
    //   version.php → 2026042400079.
    if ($oldversion < 2026042400079) {
        upgrade_mod_savepoint(true, 2026042400079, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026042800080) {
        upgrade_mod_savepoint(true, 2026042800080, 'aiknowledgecheck');
    }
    ...
    //   No DB schema changes. version.php → 2026042800081.
    if ($oldversion < 2026042800081) {
        upgrade_mod_savepoint(true, 2026042800081, 'aiknowledgecheck');
    }
    ...
    //   No DB schema changes. version.php → 2026042800082.
    if ($oldversion < 2026042800082) {
        upgrade_mod_savepoint(true, 2026042800082, 'aiknowledgecheck');
    }
    ...
    //   No DB schema changes. version.php → 2026042800083.
    if ($oldversion < 2026042800083) {
        upgrade_mod_savepoint(true, 2026042800083, 'aiknowledgecheck');
    }
    ...
    //   version.php → 2026042800084.
    if ($oldversion < 2026042800084) {
        upgrade_mod_savepoint(true, 2026042800084, 'aiknowledgecheck');
    }
    ...
    //   PHP-only fix — no JS/AMD changes, no DB schema changes. version.php → 2026042800085.
    if ($oldversion < 2026042800085) {
        upgrade_mod_savepoint(true, 2026042800085, 'aiknowledgecheck');
    }
    ...
    //   version.php → 2026042900088.
    if ($oldversion < 2026042900088) {
        upgrade_mod_savepoint(true, 2026042900088, 'aiknowledgecheck');
    }
    ...
    //   No DB schema changes. version.php → 2026042900089.
    if ($oldversion < 2026042900089) {
        upgrade_mod_savepoint(true, 2026042900089, 'aiknowledgecheck');
    }
    ...
    //   No JS, CSS, or DB schema changes. version.php → 2026042900090.
    if ($oldversion < 2026042900090) {
        upgrade_mod_savepoint(true, 2026042900090, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026042900091) {
        ...
        upgrade_mod_savepoint(true, 2026042900091, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026042900092) {
        ...
        upgrade_mod_savepoint(true, 2026042900092, 'aiknowledgecheck');
    }
    ...
    //   version.php → 2026043000093.
    if ($oldversion < 2026043000093) {
        upgrade_mod_savepoint(true, 2026043000093, 'aiknowledgecheck');
    }
    ...
    //   version.php → 2026043000094.
    if ($oldversion < 2026043000094) {
        upgrade_mod_savepoint(true, 2026043000094, 'aiknowledgecheck');
    }

    if ($oldversion < 2026050100095) {
        ...
        upgrade_mod_savepoint(true, 2026050100095, 'aiknowledgecheck');
    }

    if ($oldversion < 2026050100096) {
        ...
        upgrade_mod_savepoint(true, 2026050100096, 'aiknowledgecheck');
    }

    if ($oldversion < 2026050500106) {
        ...
        upgrade_mod_savepoint(true, 2026050500106, 'aiknowledgecheck');
    }

    if ($oldversion < 2026050500107) {
        ...
        // No DB schema changes.
        upgrade_mod_savepoint(true, 2026050500107, 'aiknowledgecheck');
    }

    if ($oldversion < 2026050500108) {
        ...
        // No DB schema changes.
        upgrade_mod_savepoint(true, 2026050500108, 'aiknowledgecheck');
    }

    // v1.5.114: FIX-CURL-BATCH — ajax.php switched all raw curl_init() calls to Moodle \curl
    //   wrapper. No DB schema changes.
    if ($oldversion < 2026051200114) {
        upgrade_mod_savepoint(true, 2026051200114, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026052700115) {
        ...
        upgrade_mod_savepoint(true, 2026052700115, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026060700117) {
        upgrade_mod_savepoint(true, 2026060700117, 'aiknowledgecheck');
    }
    ...
    //   No DB schema changes.
    if ($oldversion < 2026060900118) {
        upgrade_mod_savepoint(true, 2026060900118, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026061000120) {
        ...
        upgrade_mod_savepoint(true, 2026061000120, 'aiknowledgecheck');
    }

    if ($oldversion < 2026062200122) {
        ...
        upgrade_mod_savepoint(true, 2026062200122, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026072300222) {
        ...
        upgrade_mod_savepoint(true, 2026072300222, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026072300223) {
        ...
        upgrade_mod_savepoint(true, 2026072300223, 'aiknowledgecheck');
    }
    ...
    if ($oldversion < 2026072300224) {
        ...
        upgrade_mod_savepoint(true, 2026072300224, 'aiknowledgecheck');
    }

    if ($oldversion < 2026072400225) {
        ...
        upgrade_mod_savepoint(true, 2026072400225, 'aiknowledgecheck');
    }

    if ($oldversion < 2026072400226) {
        ...
        upgrade_mod_savepoint(true, 2026072400226, 'aiknowledgecheck');
    }

    if ($oldversion < 2026072800228) {
        ...
        upgrade_mod_savepoint(true, 2026072800228, 'aiknowledgecheck');
    }
